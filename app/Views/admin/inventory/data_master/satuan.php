<?= main_header('Inventory', 'Data Satuan', 'Inventory', 'Satuan') ?>
<?= main_periode('Daftar Satuan', 0) ?>
...
<script>
    $(document).ready(function() {
        $('#table').DataTable({
            ajax: '<?= base_url('inventory/datamaster/satuantable') ?>'
        });
    });
    ...
    $('#submit_form').submit(function(e) {
        ...
        $.ajax({
            url: "<?= base_url('inventory/datamaster/satuanadd/') ?>" + id,
            type: "post",
            data: $(this).serialize(),
            dataType: "json",
    ...
    function data_edit(id) {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('inventory/datamaster/satuanedit/') ?>" + id,
            dataType: "json",
            success: function(res) {
    ...
    function data_delete(id) {
        if (confirm('Are you sure?')) {
            $.ajax({
                type: "post",
                url: "<?= base_url('inventory/datamaster/satuandelete/') ?>" + id,
                dataType: "json",
                success: function(res) {
                    table_reload('table');
                }
