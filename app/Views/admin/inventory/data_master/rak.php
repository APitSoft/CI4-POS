<?= main_header('Inventory', 'Data Rak', 'Inventory', 'Rak') ?>
<?= main_periode('Daftar Rak', 0) ?>
...
<script>
    $(document).ready(function() {
        $('#table').DataTable({
            ajax: '<?= base_url('inventory/datamaster/raktable') ?>'
        });
    });
    ...
    $('#submit_form').submit(function(e) {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('inventory/datamaster/rakadd/') ?>" + id,
            data: $(this).serialize(),
            dataType: "json",
    ...
    function rak_edit(id) {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('inventory/datamaster/rakedit/') ?>" + id,
            dataType: "json",
            success: function(res) {
    ...
    function rak_delete(id) {
        if (confirm('Are you sure?')) {
            $.ajax({
                type: "post",
                url: "<?= base_url('inventory/datamaster/rakdelete/') ?>" + id,
                dataType: "json",
                success: function(res) {
                    table_reload('table');
                }
