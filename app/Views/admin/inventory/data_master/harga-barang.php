<?= main_header('Inventory', 'Data Harga Barang per Customer', 'Inventory', 'Harga Barang') ?>
<?= main_periode('Daftar Harga Barang per Customer', 0) ?>
<?= main_button('Harga Barang Customer Baru', 1, 0, '', 'tambahDataModal') ?>
...
<script>
    $(document).ready(function() {
        $('#table').DataTable({
            ajax: '<?= base_url('inventory/datamaster/hargabarangtable') ?>'
        });
    });
    ...
    $('#submit_form').submit(function(e) {
        ...
        $.ajax({
            url: "<?= base_url('inventory/datamaster/hargabarangadd/') ?>" + id,
            type: "post",
            data: $(this).serialize(),
            dataType: "json",
    ...
    function data_edit(id) {
        ...
        $.ajax({
            type: "post",
            url: "<?= base_url('inventory/datamaster/hargabarangedit/') ?>" + id,
            dataType: "json",
            success: function(res) {
    ...
    function data_delete(id) {
        if (confirm('Are you sure?')) {
            $.ajax({
                type: "post",
                url: "<?= base_url('inventory/datamaster/hargabarangdelete/') ?>" + id,
                dataType: "json",
                success: function(res) {
                    table_reload('table');
                }
